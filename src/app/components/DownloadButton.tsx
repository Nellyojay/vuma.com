...
export function DownloadButton({ platform, className = '' }: DownloadButtonProps) {
  const configs = {
    android: {
      text: 'Get it on',
      subtext: 'Google Play: Coming Soon',
      icon: (
  ...
  return (
    ...
    >
      ...
      <div className="text-left">
        <div className="text-white/70 text-xs">{config.text}</div>
        <div className="text-white font-semibold">{config.subtext}</div>
      </div>
    </button>
  );
}
